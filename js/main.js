...
class HealthManagementSystem {
    ...
    setupEventListeners() {
        ...
        // ダッシュボードの要注意カードをクリック
        const cautionCard = document.getElementById('cautionCard');
        if (cautionCard) {
            cautionCard.addEventListener('click', () => this.showCautionRecords());
        }
        ...
        for (let grade = 1; grade <= 6; grade++) {
            const quickBtn = document.getElementById(`quickGrade${grade}`);
            if (quickBtn) {
                quickBtn.addEventListener('click', () => this.quickFilterTodayByGrade(grade));
            }
        }
        const quickGradeAll = document.getElementById('quickGradeAll');
        if (quickGradeAll) {
            quickGradeAll.addEventListener('click', () => this.quickFilterTodayByGrade(null));
        }
        const quickCautionToday = document.getElementById('quickCautionToday');
        if (quickCautionToday) {
            quickCautionToday.addEventListener('click', () => this.showTodayCaution());
        }
    ...
    updateStatistics() {
        ...
        filteredRecords.forEach(record => {
            ...
            const hasConsultation = record.has_consultation || false;
            
            // 発熱者: 体温37.5°C以上 または 症状に「発熱」
            if (temp >= 37.5 || symptoms.includes('発熱')) {
                feverCount++;
            } 
            // 要注意: ストレス4以上 または 気分が「げんき」「ふつう」以外 または 相談希望あり
            else if (stressLevel >= 4 || 
                     (mood && mood !== '元気' && mood !== 'ふつう') || 
                     hasConsultation) {
                cautionCount++;
            } 
            // 健康良好
    ...
    quickFilterTodayByGrade(grade) {
        ...
        // 今日のデータをフィルター
        let filteredRecords = this.healthRecords.filter(record => {
            const recordDate = new Date(record.date);
        ...
        const filterHealthStatusEl = document.getElementById('filterHealthStatus');
        
        if (filterGradeEl) filterGradeEl.value = grade || '';
        if (dateFromEl) dateFromEl.value = todayStr;
        if (dateToEl) dateToEl.value = todayStr;
        if (filterStudentEl) filterStudentEl.value = '';
        if (filterHealthStatusEl) filterHealthStatusEl.value = '';
    }
    ...
    showTodayCaution() {
        ...
        if (filterGradeEl) filterGradeEl.value = '';
        if (dateFromEl) dateFromEl.value = todayStr;
        if (dateToEl) dateToEl.value = todayStr;
        if (filterStudentEl) filterStudentEl.value = '';
        if (filterHealthStatusEl) filterHealthStatusEl.value = 'caution';
    }
    ...
    loadRecordsTable() {
        const sortedRecords = this.applySorting([...this.healthRecords]);
        this.displayRecords(sortedRecords);
    }
    ...
    updateConsultationStats() {
        const newCount = this.consultations.filter(c => c.status === '未対応').length;
    ...
    displayConsultations(consultations) {
        ...
        consultations
            ...
            .forEach(consultation => {
                ...
                if (consultation.status === '未対応') statusClass = 'bg-red-100 text-red-800';
    ...
    getStatusClass(status) {
        switch (status) {
            case '未対応': return 'bg-red-100 text-red-800';
    ...
    async saveConsultationResponse() {
        ...
        try {
            ...
            console.log('元のデータ:', consultation);
            ...
            if (response.ok) {
                ...
                if (index !== -1) {
                    this.consultations[index] = { ...this.consultations[index], status: newStatus };
                }

                this.hideConsultationModal();
                this.loadConsultationsTable();
                this.updateConsultationStats();
                this.showAlert('ステータスを更新しました', 'success');
            } else {
                const errorText = await response.text();
            ...
            console.error('❌ 相談ステータス更新エラー:', error);
